...
function fetchWeatherData(city) {
    ...
    setTimeout(() => {
        ...
        const temperature = (Math.random() * 15 + 20).toFixed(1); // 20-35°C
        const humidity = (Math.random() * 40 + 40).toFixed(1); // 40-80%
        const windSpeed = (Math.random() * 5 + 1).toFixed(1); // 1-6 m/s
        const solarPotential = Math.floor(Math.random() * 30 + 70); // 70-100
        const windPotential = Math.floor(Math.random() * 40 + 40); // 40-80
...
function fetchWeatherDataByCoordinates(lat, lng) {
    // In a real application, this would be an API call to a weather service
    // For this demo, we'll simulate the data and use reverse geocoding
    ...
    // Simulate reverse geocoding and API call delay
    setTimeout(() => {
        // Generate random weather data for demo purposes
        const temperature = (Math.random() * 15 + 20).toFixed(1); // 20-35°C
        const humidity = (Math.random() * 40 + 40).toFixed(1); // 40-80%
